        ...
        <table class="table table-striped display" id="datatable">
            ...
            <c:forEach items="${users}" var="user">
                ...
                <tr id="${user.id}">
                    ...
                    <td>${user.roles}</td>
                    <td><input name="enabled" type="checkbox" value="${user.id}" <c:if test="${user.enabled}">checked</c:if> id="${user.id}"/></td>
                    <td><fmt:formatDate value="${user.registered}" pattern="dd-MMMM-yyyy"/></td>
                    <td><a><span class="glyphicon glyphicon-pencil" aria-hidden="true"></span></a></td>
                    <td><a class="delete" onclick="delete(id=${user.id})"><span class="glyphicon glyphicon-remove" aria-hidden="true"></span></a></td>
                </tr>
            </c:forEach>
        </table>
    </div>
